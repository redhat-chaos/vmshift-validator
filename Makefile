...
# Per-type VM counts. When any is set, a merged kube-burner config
# (vm-mixed.yml) is generated automatically before density-setup.
FEDORA_VMS ?=
FEDORA_HEAVY_VMS ?=
WIN_VMS ?=
...
# Skip the post-migration check step in migrate-single-vm.sh.
# Used by chaos-sweep to defer post-check until after netem cleanup.
SKIP_POST_CHECK ?=
...
MIGRATION_ARGS := \
	...
	$(if $(filter true,$(SKIP_POST_CHECK)),--skip-post-check,)
...
help: ## Show help
	...
	@echo "  make density-setup FEDORA_VMS=30 WIN_VMS=10"
	...
	@echo "  Mixed:      FEDORA_VMS  FEDORA_HEAVY_VMS  WIN_VMS"
...
render-mixed-config: ## Generate mixed-workload kube-burner config from per-type VM counts
	@$(SCRIPTS_DIR)/generate-mixed-config.sh \
		$(if $(FEDORA_VMS),--fedora $(FEDORA_VMS)) \
