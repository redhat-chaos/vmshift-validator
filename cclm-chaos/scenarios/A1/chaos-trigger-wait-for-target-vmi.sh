...
# A1 — Kill source virt-launcher when Forklift Plan reaches WaitForTargetVMI
...
# why). Resolve the target context once here so TRIGGER_CMD can use
# --context instead -- chaos-trigger.sh builds the actual merged kubeconfig
# and points krknctl's own --kubeconfig at it.
