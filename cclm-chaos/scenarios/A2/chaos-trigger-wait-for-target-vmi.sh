...
# A2 — Kill target virt-launcher when Forklift Plan reaches WaitForTargetVMI
# Thin wrapper: builds the phase-specific trigger-command and delegates to
# chaos-trigger.sh. Target node isn't known upfront (pod may not exist
# yet), so it's passed as "" — chaos-trigger.sh falls back to --pod-label
# alone, which is unique per VM.
# Usage: chaos-trigger-wait-for-target-vmi.sh <vm-name> [namespace] [mtv-namespace]

VM_NAME="${1:?Usage: $0 <vm-name> [namespace] [mtv-namespace]}"
NAMESPACE="${2:-vm-services}"
MTV_NAMESPACE="${3:-openshift-mtv}"
INJECT_PHASE="WaitForTargetVMI"
