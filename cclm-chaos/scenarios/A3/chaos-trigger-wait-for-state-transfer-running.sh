...
# A3 — Kill source virt-handler when Forklift Plan reaches WaitForStateTransfer
# with VMIM in the Running phase (live memory streaming)
# Thin wrapper: builds the phase-specific trigger-command, delegates
# injection to chaos-trigger.sh, then tracks DaemonSet respawn time.
...
TRIGGER_CMD="oc --kubeconfig=\"$TARGET_KUBECONFIG\" get plans.forklift.konveyor.io \"${VM_NAME}-migration-plan\" -n \"$MTV_NAMESPACE\" -o jsonpath='{.status.migration.vms[0].phase}' | grep -qx $INJECT_PHASE \
  && oc --kubeconfig=\"$SOURCE_KUBECONFIG\" get vmim -n \"$NAMESPACE\" -o jsonpath='{.items[0].status.phase}' | grep -qx Running"

echo "[$(ts)] Delegating to chaos-trigger.sh, gated on Forklift phase=$INJECT_PHASE + VMIM=Running (pre-vh-pod: $PRE_VH_POD)"
