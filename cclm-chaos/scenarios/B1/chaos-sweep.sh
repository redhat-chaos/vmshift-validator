...
#
# B1 — YAML-driven chaos sweep runner (latency)
#
...
#   ./chaos-sweep.sh -f iterations-brex-ng-mixed-sweep.yaml --start-from 50ms-brex-mixed-5vm
#   ./chaos-sweep.sh -f iterations-brex-ng-mixed-sweep.yaml --only smoke-50ms-brex-1vm
...
DEFAULT_ITERATIONS_FILE="${SCENARIO_DIR}/iterations-brex-ng-mixed-sweep.yaml"
...
Run B1 latency sweep iterations from a YAML file.
...
Examples:
  ...
  $(basename "$0") -f iterations-brex-ng-mixed-sweep.yaml --start-from 50ms-brex-mixed-5vm
  $(basename "$0") -f iterations-brex-ng-mixed-sweep.yaml --only smoke-50ms-brex-1vm --dry-run
...
run_iteration() {
  ...
  printf "│ Latency: %s | Interface: %s | Cooldown: %ss\n" "$latency" "$interface" "$cooldown"
  echo "└─────────────────────────────────────────────────────┘"
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    ...
    if [[ "$skip_chaos" == "true" ]]; then
      ...
      echo "  [DRY RUN] chaos: ${latency} latency on ${interface} for ${chaos_duration}s (all_workers=${all_workers})"
...
run_from_yaml() {
  ...
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo " B1 Chaos Sweep: ${sweep_name}"
  echo " ${description}"
  ...
  for ((i=0; i<count; i++)); do
    local tag name latency explicit_vm skip_chaos cooldown
    ...
    latency=$(yq_str ".iterations[$i].latency // \"0ms\"" "$ITERATIONS_FILE")
    ...
    if run_iteration \
      "$i" "$count" "$tag" "$name" "$latency" "$skip_chaos" "$vm" "$cooldown" \
...
with open(log_file) as f:
    for line in f:
        ...
        if "tag" in fields and "forklift_sec" in fields:
            ...
            # Parse latency from tag: e.g. B1-brex-ng-mixed-sweep-50ms-brex-mixed-5vm
            latency = "0ms"
            m = re.search(r'(\d+ms)', tag)
            if m:
                latency = m.group(1)
...
configurations = []
for (iface, lat), group in sorted(groups.items()):
...
for c in configurations:
    s = c["forklift_stats"]
    if s["count"] > 0:
        print(f"  {c['interface']:15s} {c['latency']:>6s}: mean={s['mean']:>6.1f}s  median={s['median']:>6.1f}s  stddev={s['stddev']:>5.1f}s  CV={s['cv_pct']:>5.1f}%  range=[{s['min']:.0f}-{s['max']:.0f}]  n={s['count']}")
PYEOF
