...
# G1 Multi-phase — IPMI node power-off during active VMIM
#
# Tests what happens when a node is abruptly powered off via IPMI while a
# cross-cluster migration (Forklift) is in progress. Unlike F1 (graceful
# drain), power-off gives no eviction, no LiveMigrate trigger — the node
# simply vanishes.
#
# Test matrix (9 tests):
#   T1. Single Fedora   — power-off source during VMIM Running
#   T2. Single Fedora   — power-off source during PrepareTarget
#   T3. Single Fedora   — power-off target during VMIM Running
#   T4. 5 parallel Fedora — power-off ONE source node
#   T5. 5 parallel Fedora — power-off ALL source nodes (power outage sim)
#   T6. Single Windows  — power-off source during VMIM Running
#   T7. 5 parallel mixed (3F+2W) — power-off ONE source node
#   T8. 5 parallel mixed (3F+2W) — power-off ALL source nodes
#   T9. 5 parallel mixed (3F+2W) — power-off ONE target node
#
# Usage:
#   bash cclm-chaos/scenarios/G1/g1-multi-phase-test.sh
#   bash cclm-chaos/scenarios/G1/g1-multi-phase-test.sh --tests T1,T2,T3
#   bash cclm-chaos/scenarios/G1/g1-multi-phase-test.sh --tests T4,T5
...
# BMC/IPMI credentials are lab-specific — never hardcode a default here.
BMC_USER="${BMC_USER:-}"
...
if [[ -z "$BMC_USER" ]] || [[ -z "$BMC_PASSWORD" ]]; then
    echo "ERROR: BMC credentials required — set \$BMC_USER and \$BMC_PASSWORD"
    exit 1
fi
...
# ── Results CSV ───────────────────────────────────────────────────────────
RESULTS_FILE="/tmp/g1-multi-phase-results-$(date +%Y%m%dT%H%M%S).csv"
echo "test_num,variant,scope,workload,vm_list,powered_off_nodes,fault_cluster,poweroff_rc,collateral_vms,kubevirt_intra_mig,source_preserved,plan_terminal,orphaned_resources,split_brain,nodes_ready,time_to_resolve_sec,run_tag" > "$RESULTS_FILE"

# ── Test definitions ──────────────────────────────────────────────────────
# Format: variant|scope|workload|run_tag
ALL_TESTS=(
    "source-sync|single|fedora|T1-single-fedora-PowerOffSource-Sync"
    "source-prepare|single|fedora|T2-single-fedora-PowerOffSource-Prepare"
    "target-sync|single|fedora|T3-single-fedora-PowerOffTarget-Sync"
    "source-sync|parallel-one-node|fedora|T4-parallel-fedora-PowerOffOneNode"
    "source-sync|parallel-all-nodes|fedora|T5-parallel-fedora-PowerOffAllNodes"
    "source-sync|single|windows|T6-single-windows-PowerOffSource-Sync"
    "source-sync|parallel-one-node|mixed|T7-parallel-mixed-PowerOffOneNode"
    "source-sync|parallel-all-nodes|mixed|T8-parallel-mixed-PowerOffAllNodes"
    "target-sync|parallel-one-node|mixed|T9-parallel-mixed-PowerOffTarget"
)
...
wait_node_ready() {
    ...
    while [[ $elapsed -lt $timeout ]]; do
        ...
        if [[ "$ready" == "True" ]]; then
            log "  Node $node is Ready (took ${elapsed}s)"
            return 0
        fi
        sleep 15
        elapsed=$((elapsed + 15))
        if (( elapsed % 60 == 0 )); then
            local power_status
            power_status=$(ipmi_power "$node" status 2>/dev/null || echo "unknown")
            log "  ... waiting for $node (${elapsed}s, power=$power_status, k8s-ready=$ready)"
        fi
    done
    log "  WARNING: Node $node NOT Ready after ${timeout}s"
    return 1
}

# ══════════════════════════════════════════════════════════════════════════
# GLOBAL NODE TRACKING — triple-redundant power restore safety
# ══════════════════════════════════════════════════════════════════════════
declare -A ALL_POWERED_OFF_NODES_MAP=()
global_power_restore() {
    echo ""
    local count=${#ALL_POWERED_OFF_NODES_MAP[@]}
    log "GLOBAL CLEANUP: Restoring power to ALL powered-off nodes ($count)..."
    if [[ $count -eq 0 ]]; then
        log "  No nodes to restore."
        return
    fi
    for node_key in "${!ALL_POWERED_OFF_NODES_MAP[@]}"; do
        IFS='|' read -r node kc <<< "$node_key"
        local status
        status=$(ipmi_power "$node" status 2>/dev/null || echo "unknown")
        if [[ "$status" == *"is on"* ]]; then
            log "  Node $node already powered on"
        else
            log "  Powering on $node..."
            ipmi_power "$node" on 2>/dev/null || true
        fi
        wait_node_ready "$node" "$kc" "$NODE_READY_TIMEOUT" || log "  WARNING: $node did not become Ready"
    done
}
trap global_power_restore EXIT
...
check_node_state() {
    ...
    local power_status
    power_status=$(ipmi_power "$node" status 2>/dev/null || echo "unknown")
    echo "ready=$ready power=$power_status"
}
...
restore_node_power() {
    ...
    wait_node_ready "$node" "$kc" "$NODE_READY_TIMEOUT" || {
        log "  WARNING: Node $node did not become Ready after power restore"
        return 1
    }
    return 0
}
...
restore_all_nodes() {
    ...
    for node in "${nodes[@]}"; do
        ...
        restore_node_power "$node" "$kc" || all_ok=false
    done
    $all_ok
}
...
for TEST_SPEC in "${TESTS[@]}"; do
    ...
    # ── Determine fault kubeconfig and chaos args ─────────────────────
    case "$VARIANT" in
        source-sync|source-prepare) FAULT_KC="$KUBECONFIG_SRC" ; FAULT_CLUSTER="source" ;;
        target-sync)                FAULT_KC="$KUBECONFIG_TGT" ; FAULT_CLUSTER="target" ;;
    esac
    ...
    CHAOS_LOG="/tmp/g1-chaos-${RUN_TAG}.log"
    CHAOS_VM="${VM_LIST[0]}"
    log "Starting chaos trigger (VM=$CHAOS_VM $CHAOS_ARGS)..."
    (
        cd "$REPO_ROOT"
        bash cclm-chaos/scenarios/G1/chaos-trigger.sh "$CHAOS_VM" $CHAOS_ARGS
    ) > "$CHAOS_LOG" 2>&1 &
    CHAOS_PID=$!
    ...
    cd "$REPO_ROOT"
    make migrate-selective VMS="$VM_CSV" MIGRATION_PROFILE=baremetal-l2 RUN_TAG="G1-${RUN_TAG}" \
      > "/tmp/g1-migration-${RUN_TAG}.log" 2>&1 &
    MIGRATION_PID=$!

    # ── Monitor until done (longer timeout: 20 min for IPMI recovery) ─
    CHAOS_FIRED="false"
    SPLIT_BRAIN_SEEN="false"
    TEST_POWERED_OFF_NODES=()

    for i in $(seq 1 240); do
        sleep 5

        # Check if chaos fired (power-off happened)
        if [[ "$CHAOS_FIRED" == "false" ]] && grep -q "FIRING: ipmitool chassis power off" "$CHAOS_LOG" 2>/dev/null; then
            CHAOS_FIRED="true"
            log "  >>> Chaos FIRED — IPMI node power-off initiated"
            while IFS= read -r powered_off; do
                [[ -n "$powered_off" ]] && TEST_POWERED_OFF_NODES+=("$powered_off")
            done < <(grep "^POWERED_OFF_NODES=" "$CHAOS_LOG" 2>/dev/null | tail -1 | sed 's/POWERED_OFF_NODES=//' | tr ' ' '\n')
        fi
    ...
    POWEROFF_RC=$(grep "^POWEROFF_RC=" "$CHAOS_LOG" 2>/dev/null | tail -1 | sed 's/POWEROFF_RC=//' || echo "?")
    ...
    if [[ ${#TEST_POWERED_OFF_NODES[@]} -eq 0 ]]; then
        POWERED_OFF_STR=$(grep "^POWERED_OFF_NODES=" "$CHAOS_LOG" 2>/dev/null | tail -1 | sed 's/POWERED_OFF_NODES=//' || echo "none")
        IFS=' ' read -ra TEST_POWERED_OFF_NODES <<< "$POWERED_OFF_STR"
    fi
    POWERED_OFF_NODES_CSV=$(IFS=';'; echo "${TEST_POWERED_OFF_NODES[*]}")

    # Track in global map for safety
    for node in "${TEST_POWERED_OFF_NODES[@]}"; do
        [[ -n "$node" ]] && ALL_POWERED_OFF_NODES_MAP["${node}|${FAULT_KC}"]=1
    done
    ...
    # ── Restore power and wait for nodes to be Ready ──────────────────
    NODES_READY="true"
    log ""
    log "Restoring power to powered-off nodes and waiting for Ready..."
    for node in "${TEST_POWERED_OFF_NODES[@]}"; do
        [[ -z "$node" || "$node" == "none" ]] && continue
        restore_node_power "$node" "$FAULT_KC" || NODES_READY="partial"
    done

    # ── Check cluster events ──────────────────────────────────────────
    log ""
    log "NodeNotReady events on $FAULT_CLUSTER cluster:"
    oc --kubeconfig "$FAULT_KC" get events --all-namespaces \
      --field-selector reason=NodeNotReady --sort-by='.lastTimestamp' 2>/dev/null | tail -5 || true
    ...
    log " VMs:                  $VM_CSV"
    log " Powered-off nodes:    $POWERED_OFF_NODES_CSV ($FAULT_CLUSTER)"
    log " Power-off RC:         $POWEROFF_RC"
    ...
    log " Nodes Ready:          $NODES_READY"
    log " Time to resolve:      ${TIME_TO_RESOLVE}s"

    # ── Record CSV ────────────────────────────────────────────────────
    echo "$TEST_NUM,$VARIANT,$SCOPE,$WORKLOAD,$VM_CSV,$POWERED_OFF_NODES_CSV,$FAULT_CLUSTER,$POWEROFF_RC,$COLLATERAL_VMS,$TOTAL_INTRA_VMIM,$SOURCE_PRESERVED,$PLAN_TERMINAL,$TOTAL_ORPHANS,$FINAL_SPLIT_BRAIN,$NODES_READY,$TIME_TO_RESOLVE,$RUN_TAG" >> "$RESULTS_FILE"
...
echo ""
echo "================================================================"
echo " G1 Multi-Phase Test Complete — IPMI Power-Off During Active VMIM"
echo "================================================================"
echo ""
...
awk -F',' 'NR>1 {
    printf "  T%-2s: variant=%-16s scope=%-20s workload=%-7s poweroff_rc=%-3s intra_mig=%-3s preserved=%-5s plan=%-5s orphans=%-3s split=%-15s nodes_ready=%-8s time=%ss\n",
...
ALL_READY=$(awk -F',' 'NR>1 && $15=="true" {n++} END {print n+0}' "$RESULTS_FILE")
...
echo "  All nodes Ready:       ${ALL_READY}/${TOTAL}"
echo ""
echo "═══ Observations ═══"
